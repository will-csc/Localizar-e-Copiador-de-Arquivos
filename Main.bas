    ...
    Else
        ...
        For Each Pasta In Folder.SubFolders
            ...
            If Folder1.SubFolders.Count > 0 Then
                Call PercorrerPastas(Folder1, Key_words, Opção_CopRem, TotalWords, CaminhoFinal)
            End If
        Next Pasta
    End If
...
Sub PercorrerPastas(Folder, Key_words, Opção_CopRem, TotalWords, CaminhoFinal)
    Set Fso = CreateObject("Scripting.FileSystemObject")
    If Folder.SubFolders.Count > 0 Then
        For Each Pasta In Folder.SubFolders
            Call AcharDoc(Pasta, Key_words, Opção_CopRem, TotalWords, CaminhoFinal)
            
            Set Folder1 = Fso.GetFolder(Pasta)
            If Folder1.SubFolders.Count > 0 Then
                For Each Pasta1 In Folder1.SubFolders
                    Call AcharDoc(Pasta1, Key_words, Opção_CopRem, TotalWords, CaminhoFinal)
                    Call PercorrerPastas(Pasta1, Key_words, Opção_CopRem, TotalWords, CaminhoFinal)
                Next Pasta1
            End If
        Next Pasta
    Else
